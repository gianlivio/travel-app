...
<div class="container my-4">
    ...
    @if($viaggio->immagine)
        <div class="text-center mb-4">
            <img src="{{ asset('storage/' . $viaggio->immagine) }}" alt="Immagine Viaggio" class="img-fluid rounded viaggio-media">
        </div>
    @else
        <p class="text-center text-muted">Nessuna Immagine disponibile</p>
    @endif

    <!-- Contenitore Flex per le informazioni e l'itinerario con layout side-by-side -->
        ...
        <div class="card flex-fill">
            ...
            <div class="card-body">
                <div class="row">
                    ...
                    <div class="col-12 mb-2">
                        <p><i class="fas fa-info-circle"></i> <strong>Dettagli:</strong></p>
                        <p>{{ $viaggio->dettagli ?? 'Nessun dettaglio disponibile' }}</p>
                    </div>
                </div>
            </div>
        </div>
                                    ...
                                    <ul class="list-group tappe-list">
                                        @foreach($giornata->tappe as $tappa)
                                            <li class="list-group-item">
                                                ...
                                                <p>{{ $tappa->descrizione ?? 'Nessuna descrizione disponibile' }}</p>
                                            </li>
                                        @endforeach
                                    </ul>
                                @else
    ...
    <div class="map-container mt-4">
        <div id="map" data-tappe='@json($tappe)'></div>
    </div>
