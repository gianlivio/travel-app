...
class ViaggioController extends Controller
{
    ...
    public function store(Request $request)
    {
        try {
            $request->validate([
                ...
                'giornate' => 'required|array', // Validazione per le giornate
                'giornate.*.data' => 'required|date',
                'giornate.*.tappe' => 'required|array', // Validazione per le tappe di ciascuna giornata
    ...
    public function update(Request $request, $id)
    {
        try {
            ...
            $viaggio->update($request->only('titolo', 'meta', 'durata', 'periodo', 'dettagli'));
            ...
            foreach ($request->input('giornate') as $giornataData) {
                if (isset($giornataData['id'])) {
                    ...
                    if (!$giornata) {
                        continue; // Se non trova la giornata, continua
                    }
                } else {
                    $giornata = $viaggio->giornate()->create([
                        'data' => $giornataData['data'],
                ...
                foreach ($giornataData['tappe'] as $tappaData) {
                    if (isset($tappaData['id'])) {
                        ...
                        if (!$tappa) {
                            continue; // Se non trova la tappa, continua
                        }
                    } else {
